...
const FAMILIES: Family[] = [
  {
    id: "plastic",
    label: "Plastic",
    description: "Fast, affordable FDM parts",
    icon: (
  ...
  },
  {
    ...
    description: "High-performance thermoplastics",
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.4} strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
  ...
  },
  {
    id: "resin",
    label: "Resin",
    description: "Ultra-fine detail SLA/MSLA",
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.4} strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
    ...
    materials: [
      { id: "std-resin",   name: "Standard Resin",  description: "Smooth surface, fine detail. Best for display models and jewelry.", tag: "best", recommended: true },
  ...
  },
  {
    id: "industrial",
    label: "Industrial",
    description: "SLS, MJF & composite printing",
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.4} strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
    ...
    materials: [
      { id: "pa11-mjf",  name: "PA 11 MJF",     description: "HP Multi Jet Fusion. Isotropic strength, fast production runs.", tag: "durable", recommended: true },
  ...
  },
  {
    id: "metal",
    label: "Metal",
    description: "DMLS / SLM metal printing",
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.4} strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
...
function Tag({ type }: { type: TagType }) {
  ...
  return (
    <span className={cn("inline-block text-[11px] font-semibold px-2 py-0.5 rounded-full border", className)}>
      {label}
    </span>
  );
}
...
function FamilyCard({
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      className={cn(
        "relative flex flex-col items-center justify-center gap-3 p-6 rounded-2xl border cursor-pointer select-none text-center transition-colors duration-150",
        selected
    ...
    >
      ...
      <div>
        <p className={cn("text-sm font-semibold", selected ? "text-accent" : "text-text-primary")}>
          {family.label}
        </p>
...
function MaterialCard({
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      className={cn(
        "relative w-full text-left flex items-start gap-4 p-5 rounded-2xl border cursor-pointer transition-colors duration-150",
        selected
    ...
    >
      ...
      {material.recommended && !selected && (
        <span className="absolute top-3 right-3 text-[10px] font-semibold text-accent/70 uppercase tracking-widest">
          Recommended
        </span>
      )}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap mb-1">
          <span className={cn("font-semibold text-sm", selected ? "text-accent" : "text-text-primary")}>
            {material.name}
          </span>
...
export function MaterialSelect({ onSelect }: MaterialSelectProps) {
  ...
  const handleConfirm = () => {
    if (selectedFamily && selectedMaterial) {
      onSelect?.(selectedFamily, selectedMaterial);
    }
  };
    ...
    <div className="w-full max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-2 mb-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={cn("text-sm font-medium", selectedFamily ? "text-text-primary" : "text-text-muted")}>
            Material Family
          </span>
        </div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-5 gap-3 mb-8">
        {FAMILIES.map((f) => (
          <FamilyCard
            key={f.id}
            family={f}
            selected={selectedFamily === f.id}
            onClick={() => handleFamilySelect(f.id)}
          />
        ))}
      </div>

      {/* ── Part B: Material list ── */}
      ...
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: selectedMaterial ? 1 : 0.3 }}
      ...
      >
        {selectedMaterial && family ? (
          ...
          >
            <span className="text-text-primary font-medium">
              {family.materials.find((m) => m.id === selectedMaterial)?.name}
            </span>
            {" "}selected
          </motion.p>
        ) : (
        ...
        <Button
          ...
          className={cn("px-10", selectedMaterial && "shadow-accent-glow")}
        >
          Confirm Material
