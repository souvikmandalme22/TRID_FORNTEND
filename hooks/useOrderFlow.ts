...
/**
 * Single hook consumed by every step component.
 * Abstracts store actions into step-specific handlers.
 */
...
export function useOrderFlow() {
  ...
  const handleModelConfirmed = (dims: { x: number; y: number; z: number }) => {
    store.setModelData({ dimensions: dims });
    goToCategory();
  };
  ...
  const handleMaterialSelect = (m: MaterialSelection) => {
    store.setMaterial(m);
    goToUseCase();
  };
  ...
  const handlePriceCalculated = (price: PriceBreakdown) => {
